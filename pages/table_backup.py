...
layout = html.Div([
    ...
    dbc.Row([
        dbc.Col(dcc.Input(id='input-row-id', type='number', placeholder='Row ID')),
        dbc.Col(dcc.Input(id='input-order-id', type='text', placeholder='Order ID')),
        dbc.Col(dcc.DatePickerSingle(id='input-order-date', placeholder='Order Date')),
        dbc.Col(dcc.Input(id='input-customer-name', type='text', placeholder='Customer Name')),
        dbc.Col(dcc.Input(id='input-segment', type='text', placeholder='Segment')),
        dbc.Col(dcc.Input(id='input-category', type='text', placeholder='Category')),
        dbc.Col(dcc.Input(id='input-sub-category', type='text', placeholder='Sub-Category')),
        dbc.Col(dcc.Input(id='input-product-name', type='text', placeholder='Product Name')),
        dbc.Col(dcc.Input(id='input-country-region', type='text', placeholder='Country/Region')),
        dbc.Col(dcc.Input(id='input-state-province', type='number', placeholder='State/Province')),
        dbc.Col(dcc.Input(id='input-city', type='text', placeholder='City')),
        dbc.Col(dcc.Input(id='input-sale', type='text', placeholder='Sale')),
        dbc.Col(dcc.Input(id='input-profit', type='text', placeholder='Profit')),
        dbc.Col(dcc.Input(id='input-quantity', type='text', placeholder='Quantity')),
    ]),
...
#     Output('add-row-response', 'children'),
#     Input('add-row-button', 'n_clicks'),
#     State('datatable', 'data'),
#     State('input-row-id', 'value'),
...
#     State('input-country-region', 'value'),
#     State('input-state-province', 'value'),
#     State('input-city', 'value'),
#     State('input-sale', 'value'),
#     State('input-profit', 'value'),
#     State('input-quantity', 'value'),
#     prevent_initial_call=True,
#     allow_duplicate=True
# )
# def add_row(n_clicks, rows, input_row_id, input_order_id, input_order_date, 
#             input_customer_name, input_segment, input_category, input_sub_category,
#             input_product_name, input_country_region, input_state_province, 
#             input_city, input_sale, input_profit, input_quantity):
    
#     if n_clicks > 0:
#         # Check if the Row ID already exists
#         existing_ids = {row['Row ID'] for row in rows}
#         if input_row_id in existing_ids:
#             return rows, "Error: Duplicate Row ID."
...
#         pd.DataFrame(rows).to_csv('updated_superstore_data.csv', index=False)

#         return rows, "Row added successfully."
    ...
    if trigger_id == 'add-row-button':
        ...
        if n_clicks > 0:
            ...
            if input_row_id in existing_ids:
                ...
                return rows, selected_size, "Error: Duplicate Row ID."
...
def update_table_content(selected_sub_category, selected_segment, selected_category, selected_country, 
                         selected_state, selected_city, selected_size, start_date, end_date, n_clicks):
    
    filtered_df = df

    if n_clicks > 0:
        print("Get Add button")
        new_row = {col_id: "" for col_id in COLUMNS}  # Create an empty row

        print(f"New Row: {new_row}")
        print(f"filtered data columns: {filtered_df.columns}")
        filtered_df.loc[len(filtered_df)] = new_row
        filtered_df['Order Date'] = pd.to_datetime(df['Order Date'])
...
def update_table_content(selected_sub_category, selected_segment, selected_category, selected_country, 
    ...
    if ctx.triggered:
        ...
        if trigger_id == 'add-row-button' and n_clicks > 0:
            # Validate inputs
            if not all([input_row_id, input_order_id, input_order_date, input_customer_name]):
            ...
            new_row = {
                'Row ID': input_row_id,
                'Order ID': input_order_id,
                'Order Date': pd.to_datetime(input_order_date),
                'Customer Name': input_customer_name,
                'Segment': input_segment,
                'Category': input_category,
                'Sub-Category': input_sub_category,
    ...
    # Apply filtering based on the inputs...

    
    return filtered_df.to_dict('records'), selected_size, "Data updated."